...
function LineChartSVG2({ series, width = 980, height = 280, padding = 44 }) {
  ...
  // linha zero (se 0 estiver no range)
  const y0 = scaleY(Math.max(minY, 0));
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Gráfico efetivo x projetado">
      <defs>
...
export default function RelatorioFluxoCaixaProjetado() {
  ...
  async function gerar() {
    ...
    try {
      ...
      const resp = await apiFetch(`/relatorios/fluxo-caixa/projetado?${qs.toString()}`);
      setData(resp);
  ...
  const diasRiscoEf = safeArray(data?.diasRiscoEfetivo);
  const diasRiscoPr = safeArray(data?.diasRiscoProjetado);
    ...
    <>
      ...
      <div className="p-6">
        <div className="no-print flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl font-bold text-slate-900">Relatório — Fluxo de Caixa Projetado</h1>
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
          {!data ? (
            ...
            <>
              <div className="text-sm font-bold text-slate-900 mb-2">Saldo diário: efetivo x projetado</div>
              <LineChartSVG2 series={serie} />

              <div className="mt-3 text-sm text-slate-800 flex flex-wrap gap-4">
                <div>
                  <b>Dias de risco (efetivo):</b> {diasRiscoEf.length}
                </div>
                <div>
                  <b>Dias de risco (projetado):</b> {diasRiscoPr.length}
                </div>
              </div>

              <div className="mt-2 text-xs text-slate-600">
                “Risco” = saldo negativo no fim do dia.
              </div>
            </>
          )}
        </div>
        ...
        <div id="print-area" className="print-only">
          <div className="page">
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, marginTop: 4 }}>Fluxo de Caixa — Projetado</div>
            </div>
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ fontWeight: 800, marginBottom: 6, fontSize: 12 }}>GRÁFICO — EFETIVO X PROJETADO</div>
              <LineChartSVG2 series={serie} />
              <div style={{ marginTop: 10, fontSize: 11, color: "#444" }}>
                Risco (saldo negativo no fim do dia) — Efetivo: <b>{diasRiscoEf.length}</b> • Projetado: <b>{diasRiscoPr.length}</b>
              </div>
            </div>
