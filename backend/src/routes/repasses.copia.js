...
import { enviarPix, detectarTipoChave } from "../lib/interPix.js";
...
function buildEmailRepasseRealizado(
  nomeAdvogado,
  {
    competenciaMes,
    competenciaAno,
    ...
    saldoUtilizadoCentavos,
    adiantamentoCentavos,
    valorTransferidoCentavos,
    emprestimosPagosCentavos,
  }
) {
  const fmtBRL = (c) => (c / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
  const mesNome = _MESES_PT[competenciaMes - 1] || String(competenciaMes);
  const dataStr = dataRepasse ? _fmtDatePT(dataRepasse) : "\u2014";
  const valorCalculado = Number(valorCalculadoCentavos || 0);
  const saldoUtilizado = Number(saldoUtilizadoCentavos || 0);
  const adiantamento = Number(adiantamentoCentavos || 0);
  const valorTransferido = Number(valorTransferidoCentavos || 0);
  const emprestimosPagos = Number(emprestimosPagosCentavos || 0);

  const rowSaldoUtilizado = saldoUtilizado !== 0
    ? `<tr>
            <td style="padding:6px 0;font-size:12px;color:#64748b;font-weight:600;text-transform:uppercase">Saldo utilizado</td>
            <td style="padding:6px 0;font-size:14px;color:#0f172a">${fmtBRL(saldoUtilizado)}</td>
          </tr>`
    : "";

  const rowAdiantamento = adiantamento !== 0
    ? `<tr>
            <td style="padding:6px 0;font-size:12px;color:#64748b;font-weight:600;text-transform:uppercase">Adiantamento</td>
            <td style="padding:6px 0;font-size:14px;color:#0f172a">${fmtBRL(adiantamento)}</td>
          </tr>`
    : "";

  const rowEmprestimosPagos = emprestimosPagos !== 0
    ? `<tr>
            <td style="padding:6px 0;font-size:12px;color:#64748b;font-weight:600;text-transform:uppercase">Empr\u00e9stimos pagos (compet\u00eancia)</td>
            <td style="padding:6px 0;font-size:14px;color:#0f172a">${fmtBRL(emprestimosPagos)}</td>
          </tr>`
    : "";
  ...
  <div style="max-width:640px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)">
    ...
    <div style="padding:24px 28px">
      <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:16px 20px;margin-bottom:20px">
        <table style="width:100%;border-collapse:collapse">
          <tr>
            <td style="padding:6px 0;font-size:12px;color:#64748b;font-weight:600;text-transform:uppercase;width:240px">Compet\u00eancia</td>
          ...
          <tr>
            <td style="padding:6px 0;font-size:12px;color:#64748b;font-weight:600;text-transform:uppercase">Repasse calculado</td>
            <td style="padding:6px 0;font-size:14px;color:#0f172a">${fmtBRL(valorCalculado)}</td>
          </tr>
          ${rowSaldoUtilizado}
          ${rowAdiantamento}
          <tr style="background:#dbeafe">
            <td style="padding:8px 0;font-size:12px;color:#1e3a8a;font-weight:700;text-transform:uppercase">Transferido</td>
            <td style="padding:8px 0;font-size:18px;font-weight:800;color:#1e40af">${fmtBRL(valorTransferido)}</td>
          </tr>
          ${rowEmprestimosPagos}
        </table>
      </div>
...
function buildWhatsAppRepasseRealizadoMessage(
  nomeAdvogado,
  {
    competenciaMes,
    competenciaAno,
  ...
  const anoStr = String(competenciaAno || "");
  const saldoUtilizado = Number(saldoUtilizadoCentavos || 0);
  const adiantamento = Number(adiantamentoCentavos || 0);
  const emprestimosPagos = Number(emprestimosPagosCentavos || 0);

  const linhas = [
    `\u2705 *Repasse realizado \u2014 ${mesStr}/${anoStr}*`,
    "",
    `Ol\u00e1, *${nomeAdvogado}*!`,
    "",
    `Repasse Calculado: ${fmtBRL(valorCalculadoCentavos)}`,
  ];

  if (saldoUtilizado !== 0) {
    linhas.push(`Saldo Utilizado: ${fmtBRL(saldoUtilizado)}`);
  }

  if (adiantamento !== 0) {
    linhas.push(`Adiantamento: ${fmtBRL(adiantamento)}`);
  }

  linhas.push(`*Transferido: ${fmtBRL(valorTransferidoCentavos)}*`);

  if (emprestimosPagos !== 0) {
    linhas.push(`Empr\u00e9stimos pagos (compet\u00eancia): ${fmtBRL(emprestimosPagos)}`);
  }

  linhas.push("", "Em caso de d\u00favidas, fale com o financeiro.");
  return linhas.join("\n");
}
// ============================================================
  ...
  try {
    ...
    for (const [advogadoId, registro] of porAdvogado.entries()) {
      ...
      registro.parcelaFixaValorCentavos = advogado.parcelaFixaValor
        ...
        : 0;
      registro.chavePix = advogado.chavePix || null;
    ...
    const items = Array.from(porAdvogado.values()).map(registro => ({
      ...
      chavePix: registro.chavePix || null,
...
router.post("/api/repasses/realizar", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const repasseNotificacao = result.repasseRealizado;
    const competenciaMesNotificacao = repasseNotificacao.competenciaMes ?? mes;
    const competenciaAnoNotificacao = repasseNotificacao.competenciaAno ?? ano;
    const emprestimosPagosRows = await prisma.livroCaixaLancamento.findMany({
      where: {
        competenciaAno: Number(competenciaAnoNotificacao),
        competenciaMes: Number(competenciaMesNotificacao),
    ...
    const emprestimosPagosCentavos = emprestimosPagosRows.reduce((sum, row) => (
      sum + Number(row?.valorCentavos || 0)
    ), 0);
    const dadosRepasseNotificacao = {
      competenciaMes: competenciaMesNotificacao,
      competenciaAno: competenciaAnoNotificacao,
      dataRepasse: dataRepasseDate,
      valorCalculadoCentavos: Number(repasseNotificacao.valorPrevistoTotalCentavos || 0),
      saldoUtilizadoCentavos: Number(repasseNotificacao.saldoConsumidoCentavos || 0),
      adiantamentoCentavos: Number(valorAbatimentoTotalCentavos || 0),
      valorTransferidoCentavos: Math.max(0, Number(cashRepasseCentavos || 0)),
      emprestimosPagosCentavos,
    };

    {
      const subjectRepasse = `\u2705 Addere \u2014 Repasse processado: ${_MESES_PT[dadosRepasseNotificacao.competenciaMes - 1]} de ${dadosRepasseNotificacao.competenciaAno} \u2014 ${advogado.nome}`;
      (async () => {
        try {
          if (advogado?.ativo && advogado?.email) {
            await sendEmail({
              ...
              html: buildEmailRepasseRealizado(advogado.nome, dadosRepasseNotificacao),
            });
          }
    ...
    // ── Envio de Pix automático (fire-and-forget) ─────────────────────────────
    if (enviarPixAposRealizar && advogado.chavePix && cashRepasseCentavos > 0) {
      ;(async () => {
        try {
          // Verificar se a conta selecionada é Inter (tem interContaId preenchido)
          const contaEfetiva = contaIdRepasse ? parseInt(contaIdRepasse) : (splitArr?.[0]?.contaId || null);
          let isContaInter = false;
          if (contaEfetiva) {
            const conta = await prisma.livroCaixaConta.findUnique({
              where: { id: contaEfetiva },
              select: { interContaId: true },
            });
            isContaInter = !!(conta?.interContaId);
          }

          if (!isContaInter) {
            console.log("ℹ️ Pix não enviado: conta selecionada não é Inter PJ");
            return;
          }

          const competencia = `${String(result.repasseRealizado.competenciaMes).padStart(2, "0")}/${result.repasseRealizado.competenciaAno}`;
          const interResp = await enviarPix({
            chavePix:          advogado.chavePix,
            valorCentavos:     cashRepasseCentavos,
            descricao:         `Repasse ${competencia} — ${advogado.nome}`,
            favorecidoNome:    advogado.nome,
            cpfCnpjFavorecido: advogado.cpf || null,
          });

          await prisma.pixPagamento.create({
            data: {
              endToEndId:     interResp.endToEndId || null,
              chavePix:       advogado.chavePix,
              tipoChave:      detectarTipoChave(advogado.chavePix),
              favorecidoNome: advogado.nome,
              valorCentavos:  cashRepasseCentavos,
              descricao:      `Repasse ${competencia}`,
              status:         interResp.status || "PROCESSANDO",
              repasseId:      result.repasseRealizado.id,
              advogadoId:     parseInt(advogadoId),
          ...
          console.log(`💸 Pix enviado automaticamente para ${advogado.nome}: R$ ${(cashRepasseCentavos / 100).toFixed(2)}`);
        } catch (pixErr) {
          console.error("❌ Pix automático pós-repasse falhou:", pixErr.message);
          // Não rejeita — o repasse já foi realizado com sucesso
        }
      })();
    }
    ...
    ;(async () => {
      try {
        ...
        if (!phone) return;
        const fmtVal = (c) => (Number(c || 0) / 100).toLocaleString("pt-BR", { minimumFractionDigits: 2 });
        const mesStr = String(dadosRepasseNotificacao.competenciaMes || mes).padStart(2, "0");
        const msg = buildWhatsAppRepasseRealizadoMessage(advogado.nome, dadosRepasseNotificacao);
        try {
          ...
          await sendWhatsAppTemplate(phone, "realizado_repasse", "pt_BR", [{
            ...
            parameters: [
              ...
              { type: "text", text: String(dadosRepasseNotificacao.competenciaAno || ano) },
              { type: "text", text: fmtVal(dadosRepasseNotificacao.valorTransferidoCentavos) },
            ],
          }]);
        }
      } catch (_) {}
...
router.get("/api/relatorios/repasses", authenticate, async (req, res) => {
  try {
    ...
    const parseAbatimentosObservacao = (obs) => {
      const out = [];
      ...
      for (const part of parts) {
        const mRepasse = part.match(/Repasse\s*#\s*(\d+)/i);
        const mAbatido = part.match(/Abatido\s+R\$\s*([0-9.]+,[0-9]{2})/i);
        if (!mRepasse || !mAbatido) continue;
        const repasseId = Number(mRepasse[1]);
        const valorCentavos = brlToCent(mAbatido[1]);
        if (!repasseId || valorCentavos <= 0) continue;
        out.push({ repasseId, valorCentavos });
      }
      return out;
    };
    const parseMovimentosAdiantamentoObservacao = (obs) => {
      const out = [];
      const parts = String(obs || "")
        .split("|")
        .map((s) => s.trim())
      ...
      for (const part of parts) {
        const mAbatido = part.match(/Abatido\s+R\$\s*([0-9.]+,[0-9]{2})/i);
        const mCompetencia = part.match(/Compet\S*\s*(\d{1,2})\/(\d{4})/i);
        if (!mAbatido || !mCompetencia) continue;
        const valorCentavos = brlToCent(mAbatido[1]);
        const mesMov = Number(mCompetencia[1]);
        const anoMov = Number(mCompetencia[2]);
        if (valorCentavos <= 0 || !mesMov || !anoMov) continue;
        out.push({ ano: anoMov, mes: mesMov, valorCentavos });
      }
      return out;
    };
    const ymLTE = (anoA, mesA, anoB, mesB) => (
      Number(anoA) < Number(anoB) || (Number(anoA) === Number(anoB) && Number(mesA) <= Number(mesB))
    );
    const ymEQ = (anoA, mesA, anoB, mesB) => (
      Number(anoA) === Number(anoB) && Number(mesA) === Number(mesB)
    );
    ...
    for (const adv of advogados) {
      repassesPorAdv.set(adv.id, {
        itens: [],
        totalCent: 0,
        totalCalculadoCent: 0,
        totalEfetivadoCent: 0,
        totalSaldoGeradoCent: 0,
        totalSaldoConsumidoCent: 0,
        totalAbatimentoAdiantamentoCent: 0,
        totalTransferenciaRepasseLCCent: 0,
        totalParcelaFixaLCCent: 0,
        totalTransferenciaParaAdvogadoLCCent: 0,
        totalEmprestimoEntradaLCCent: 0,
        totalEmprestimoSaidaLCCent: 0,
        totalImpactoLiquidoCaixaLCCent: 0,
        saldoPosteriorCentavos: null,
        saldoPosteriorRef: null,
        saldoAnteriorCentavos: saldoAnteriorMap.get(adv.id) || 0,
      });
    }
    ...
    const repasseIds = repasses.map((r) => Number(r.id));
    const repasseIdsSet = new Set(repasseIds);

    // Mapeia saídas/entradas do LC na competência para explicar "valor transferido"
    const lcCompetencia = await prisma.livroCaixaLancamento.findMany({
      where: {
        competenciaAno: ano,
        competenciaMes: mes,
        origem: "REPASSES_REALIZADOS",
    ...
    const repasseTransferLcPorRepasseId = new Map();
    const parcelaFixaLcPorAdv = new Map();
    const emprestimoLcEntradaPorAdv = new Map();
    const emprestimoLcSaidaPorAdv = new Map();
    const addToMap = (map, key, value) => {
      map.set(key, (map.get(key) || 0) + Number(value || 0));
    };

    for (const lc of lcCompetencia) {
      const ref = String(lc?.referenciaOrigem || "").trim();
      const es = String(lc?.es || "").trim().toUpperCase();
      const valorCentavos = Number(lc?.valorCentavos || 0);
      if (!ref || valorCentavos === 0) continue;

      const mRep = ref.match(/^REPASSE_(\d+)(?:_SPLIT_\d+)?$/);
      if (mRep) {
        const repasseId = Number(mRep[1]);
        if (es === "S" && repasseIdsSet.has(repasseId)) {
          addToMap(repasseTransferLcPorRepasseId, repasseId, valorCentavos);
        }
        continue;
      }

      const mParcelaFixa =
        ref.match(/^PARCELA_FIXA_REPASSE_(\d+)_(\d{4})_(\d{1,2})$/) ||
        ref.match(/^REPASSE_PARCELA_FIXA_SOMADA_(\d+)_(\d{4})_(\d{1,2})$/);
      if (mParcelaFixa) {
        const advId = Number(mParcelaFixa[1]);
        const anoRef = Number(mParcelaFixa[2]);
        const mesRef = Number(mParcelaFixa[3]);
        if (anoRef === ano && mesRef === mes && es === "S") {
          addToMap(parcelaFixaLcPorAdv, advId, valorCentavos);
        }
        continue;
      }

      const mEmprestimo = ref.match(/^EMPRESTIMO_SOCIO_(\d+)_(\d{4})_(\d{1,2})$/);
      if (mEmprestimo) {
        const advId = Number(mEmprestimo[1]);
        const anoRef = Number(mEmprestimo[2]);
        const mesRef = Number(mEmprestimo[3]);
        if (anoRef !== ano || mesRef !== mes) continue;
        if (es === "E") addToMap(emprestimoLcEntradaPorAdv, advId, valorCentavos);
        if (es === "S") addToMap(emprestimoLcSaidaPorAdv, advId, valorCentavos);
      }
    }

    const adiantamentosObservacao = repasseIds.length
      ? await prisma.adiantamentoSocio.findMany({
          where: {
            advogadoId: { in: advogadoIds },
            observacoes: { not: null },
            OR: repasseIds.map((id) => ({
              observacoes: { contains: `Repasse #${id}`, mode: "insensitive" },
            })),
          },
          select: { id: true, observacoes: true },
        })
      : [];
    const abatimentoPorRepasseId = new Map();
    for (const adt of adiantamentosObservacao) {
      const abatimentos = parseAbatimentosObservacao(adt.observacoes);
      for (const item of abatimentos) {
        if (!repasseIdsSet.has(item.repasseId)) continue;
        addToMap(abatimentoPorRepasseId, item.repasseId, item.valorCentavos);
      }
    }
    ...
    for (const rr of repasses) {
      ...
      const saldoGeradoCent = Number(rr?.saldoGeradoCentavos || 0);
      const saldoConsumidoCent = Number(rr?.saldoConsumidoCentavos || 0);
      const transferenciaRepasseLCCent = Number(repasseTransferLcPorRepasseId.get(Number(rr.id)) || 0);
      const abatimentoAdiantamentoCent = Number(abatimentoPorRepasseId.get(Number(rr.id)) || 0);
      ...
      pack.saldoPosteriorRef = rr?.dataRepasse || rr?.createdAt || pack.saldoPosteriorRef || null;

      pack.totalCalculadoCent += somaPrevisto;
      pack.totalEfetivadoCent += efetivadoCent;
      pack.totalSaldoGeradoCent += saldoGeradoCent;
      pack.totalSaldoConsumidoCent += saldoConsumidoCent;
      pack.totalAbatimentoAdiantamentoCent += abatimentoAdiantamentoCent;
      pack.totalTransferenciaRepasseLCCent += transferenciaRepasseLCCent;

      // Regra do detalhamento:
      // - Se efetivado < calculado: rateia proporcionalmente a diferença negativa
      // - Se efetivado >= calculado: mantém valores originais por parcela (sem rateio positivo)
      const deveRatearDetalhamento = somaPrevisto > 0 && efetivadoCent < somaPrevisto;
      ...
      for (let i = 0; i < lancs.length; i++) {
        ...
        let recebidoCent = 0;
        if (somaPrevisto > 0 && deveRatearDetalhamento) {
          // proporcional apenas quando faltou efetivação
          recebidoCent = Math.round((efetivadoCent * previstoCent) / somaPrevisto);
          acumuladoRateio += recebidoCent;
        } else if (somaPrevisto > 0) {
          // não distribui excedente de saldo utilizado no detalhamento
          recebidoCent = previstoCent;
        } else {
          recebidoCent = 0;
        }
      ...
      // corrige arredondamento somente quando houve rateio negativo
      if (deveRatearDetalhamento) {
    ...
    for (const adv of advogados) {
      ...
      const pfCent = Number(parcelaFixaLcPorAdv.get(advId) || 0);
      const pfPack = parcelasFixasPorAdv.get(advId);
      if (pfPack && pfCent > 0) {
        pfPack.itens.push({
          nome: pfPack.nome,
          dataRecebimento: null,
          valorCentavos: pfCent,
        });
        pfPack.totalCent += pfCent;
      }

      const repPack = repassesPorAdv.get(advId);
      if (!repPack) continue;
      repPack.totalParcelaFixaLCCent += pfCent;
      repPack.totalEmprestimoEntradaLCCent += Number(emprestimoLcEntradaPorAdv.get(advId) || 0);
      repPack.totalEmprestimoSaidaLCCent += Number(emprestimoLcSaidaPorAdv.get(advId) || 0);
      repPack.totalTransferenciaParaAdvogadoLCCent =
        repPack.totalTransferenciaRepasseLCCent + repPack.totalParcelaFixaLCCent;
      repPack.totalImpactoLiquidoCaixaLCCent =
        repPack.totalTransferenciaParaAdvogadoLCCent +
        repPack.totalEmprestimoSaidaLCCent -
        repPack.totalEmprestimoEntradaLCCent;
    }
    ...
    // Empréstimos:
    // - pendentes até a competência selecionada
    // - quitados dentro da competência selecionada (para evidenciar o pagamento no mês)
    const inicioCompetencia = new Date(ano, mes - 1, 1, 0, 0, 0, 0);
    const inicioProximaCompetencia = new Date(ano, mes, 1, 0, 0, 0, 0);
    ...
    // Adiantamentos pendentes contextualizados até a competência selecionada
    const adiantamentosRows = await prisma.adiantamentoSocio.findMany({
      ...
      include: { cliente: { select: { nomeRazaoSocial: true } } },
      orderBy: [{ competenciaAno: 'asc' }, { competenciaMes: 'asc' }, { dataRegistro: 'asc' }],
    });

    const adiantamentosPorAdv = new Map(); // advId -> []
    const fimCompetencia = new Date(ano, mes, 0, 23, 59, 59, 999);
    for (const a of adiantamentosRows) {
      const valorAdiantadoCentavos = Number(a.valorAdiantadoCentavos || 0);
      const valorDevolvidoAtualCentavos = Number(a.valorDevolvidoCentavos || 0);
      const movimentos = parseMovimentosAdiantamentoObservacao(a.observacoes);

      let devolvidoAteCompetenciaCentavos = movimentos
        .filter((m) => ymLTE(m.ano, m.mes, ano, mes))
        .reduce((sum, m) => sum + Number(m.valorCentavos || 0), 0);

      if (devolvidoAteCompetenciaCentavos <= 0 && valorDevolvidoAtualCentavos > 0) {
        const dataMarcador = a.dataQuitacao || a.updatedAt || null;
        if (dataMarcador && new Date(dataMarcador) <= fimCompetencia) {
          devolvidoAteCompetenciaCentavos = valorDevolvidoAtualCentavos;
        }
      }

      devolvidoAteCompetenciaCentavos = Math.min(
        valorDevolvidoAtualCentavos,
        Math.max(0, devolvidoAteCompetenciaCentavos),
      );

      const saldoPendenteCentavos = Math.max(0, valorAdiantadoCentavos - devolvidoAteCompetenciaCentavos);

      const entradaCompetenciaCentavos = ymEQ(a.competenciaAno, a.competenciaMes, ano, mes)
        ? valorAdiantadoCentavos
        : 0;

      let saidaCompetenciaCentavos = movimentos
        .filter((m) => ymEQ(m.ano, m.mes, ano, mes))
        .reduce((sum, m) => sum + Number(m.valorCentavos || 0), 0);

      if (saidaCompetenciaCentavos <= 0 && valorDevolvidoAtualCentavos > 0) {
        const dataMarcador = a.dataQuitacao || a.updatedAt || null;
        if (dataMarcador) {
          const markerDate = new Date(dataMarcador);
          if (ymEQ(markerDate.getFullYear(), markerDate.getMonth() + 1, ano, mes)) {
            saidaCompetenciaCentavos = devolvidoAteCompetenciaCentavos;
          }
        }
      }

      const saidaCompetenciaNormalizada = Math.min(
        Math.max(0, saidaCompetenciaCentavos),
        devolvidoAteCompetenciaCentavos,
      );
      const possuiMovimentoCompetencia = entradaCompetenciaCentavos > 0 || saidaCompetenciaNormalizada > 0;
      if (saldoPendenteCentavos <= 0 && !possuiMovimentoCompetencia) continue;
      ...
      adiantamentosPorAdv.get(a.advogadoId).push({
        ...
        valorAdiantadoCentavos,
        valorDevolvidoCentavos: devolvidoAteCompetenciaCentavos,
        entradaCompetenciaCentavos,
        saidaCompetenciaCentavos: saidaCompetenciaNormalizada,
    ...
    const out = advogados.map((adv) => {
      ...
      return {
        ...
        totalRepassesCalculadoCentavos: Number(rep.totalCalculadoCent || 0),
        totalRepassesEfetivadosCentavos: Number(rep.totalEfetivadoCent || 0),
        saldoPosteriorCentavos: Number(rep.saldoPosteriorCentavos ?? 0),
        saldoAnteriorCentavos: Number(rep.saldoAnteriorCentavos ?? 0),
        resumoFinanceiro: {
          repasseCalculadoCentavos: Number(rep.totalCalculadoCent || 0),
          repasseEfetivadoCentavos: Number(rep.totalEfetivadoCent || 0),
          transferenciaRepasseLCCentavos: Number(rep.totalTransferenciaRepasseLCCent || 0),
          parcelaFixaLCCentavos: Number(rep.totalParcelaFixaLCCent || 0),
          transferenciaParaAdvogadoLCCentavos: Number(rep.totalTransferenciaParaAdvogadoLCCent || 0),
          adiantamentoAbatidoCentavos: Number(rep.totalAbatimentoAdiantamentoCent || 0),
          saldoGeradoCentavos: Number(rep.totalSaldoGeradoCent || 0),
          saldoConsumidoCentavos: Number(rep.totalSaldoConsumidoCent || 0),
          ajusteSaldoCentavos: Number((rep.totalSaldoConsumidoCent || 0) - (rep.totalSaldoGeradoCent || 0)),
          emprestimoEntradaLCCentavos: Number(rep.totalEmprestimoEntradaLCCent || 0),
          emprestimoSaidaLCCentavos: Number(rep.totalEmprestimoSaidaLCCent || 0),
          impactoLiquidoCaixaLCCentavos: Number(rep.totalImpactoLiquidoCaixaLCCent || 0),
        },
