...
router.post("/api/repasses/realizar", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    // WhatsApp — repasse_realizado_detalhado para advogado
    ;(async () => {
      try {
        const phone = _waPhone(advogado.telefone);
        if (!phone) return;
        const fmtVal = (c) => ((c || 0) / 100).toLocaleString("pt-BR", { minimumFractionDigits: 2 });
        const mesStr = String(repasse.competenciaMes || mes).padStart(2, "0");
        const anoStr = String(repasse.competenciaAno || ano);

        // Empréstimos quitados na competência (dataQuitacao dentro do mês/ano)
        const startComp = new Date(parseInt(ano), parseInt(mes) - 1, 1);
        const endComp   = new Date(parseInt(ano), parseInt(mes), 0, 23, 59, 59, 999);
        const emprestimosComp = await prisma.emprestimoSocio.findMany({
          where: {
            advogadoId: parseInt(advogadoId),
            quitado:    true,
            dataQuitacao: { gte: startComp, lte: endComp },
          },
        });
        const empCentavos = emprestimosComp.reduce((s, e) => s + (e.valorCentavos || 0), 0);

        await sendWhatsAppTemplate(phone, "realizado_repasse_detalhado", "pt_BR", [
          {
            type: "body",
            parameters: [
              { type: "text", text: `${mesStr}/${anoStr}` },          // {{1}} competência
              { type: "text", text: advogado.nome },                   // {{2}} nome
              { type: "text", text: fmtVal(valorPrevistoTotalCentavos) }, // {{3}} rep. calculado
              { type: "text", text: fmtVal(repasse.saldoConsumidoCentavos) }, // {{4}} saldo utilizado
              { type: "text", text: fmtVal(valorAbatimentoTotalCentavos) },   // {{5}} adiantamento
              { type: "text", text: fmtVal(cashRepasseCentavos) },     // {{6}} transferido
              { type: "text", text: fmtVal(empCentavos) },             // {{7}} empréstimos pagos
            ],
          },
        ]);
      } catch (_) {}
